...
fn check_for_adjacent_symbol(
    schema: [[char; SCHEMA_SIZE]; SCHEMA_SIZE],
    i: usize,
    j: usize,
) -> bool {
    ...
    // check row below
    if SYMBOLS.contains(schema[i + 1][j - 1])
        || SYMBOLS.contains(schema[i + 1][j])
        || SYMBOLS.contains(schema[i + 1][j + 1])
    {
        is_part = true
    }
